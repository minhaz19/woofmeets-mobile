...
const ForceUpdates = () => {
    ...
    const checkIfUpdateIsNeeded = async () => {
        const response = await methods._get(`user-application-version/compare?platform=${Platform.OS.toUpperCase()}&version=${currentVersion}`);
        if (response?.data?.data?.isForceUpdate) {
            Alert.alert(
                ...
                [
                  { text: 'Update', onPress: () => {
                    Linking.openURL(response?.data?.data?.storeRedirectUrl);
                    BackHandler.exitApp();
                    RNExitApp.exitApp();
                },
                },
